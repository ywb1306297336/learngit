...
@interface WMUTestOneController () <UINavigationControllerDelegate>

@end

@implementation WMUTestOneController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [self.tabBarController tabBarHide:YES time:1.0 offsetOrigin:CGPointMake(0, WMUScreenH)];
}
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"测试一";
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self.tabBarController tabBarHide:NO time:1.0 offsetOrigin:CGPointMake(0, WMUScreenH)];
}
#pragma mark - 监听
...
- (id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC {
    if (operation == UINavigationControllerOperationPop) {
        return [[WMUBottomPop alloc] init];
    }
    return nil;
}
